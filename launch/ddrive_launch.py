...
def generate_launch_description():
    ...
    urdf_path = get_package_share_path('diff_drive')
    default_model_path = urdf_path / 'ddrive.urdf.xacro'

    package_share = os.path.join(get_package_share_directory('diff_drive'))
    ...
    start_gazebo = IncludeLaunchDescription(PythonLaunchDescriptionSource(
        ...
        launch_arguments={'ign_args': [os.path.join(get_package_share_directory(
            'diff_drive')), '/ddrive.world -r']}.items())
    ...
    flip = Node(package='diff_drive', executable='flip')
